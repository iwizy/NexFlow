...
| Security policy composition | Draft operation scope, independent policy intersection, revalidation, untrusted content, bounded work, and failure review model | Specified | [Security Model](security-model.md), [RFC-0022](../rfcs/RFC-0022-security-policy-composition.md) | Draft proposal and manual scenarios only; no complete policy evaluator, principal authentication, isolation, live approval, revocation, or uncertain-effect enforcement. |
...
| GitHub extension draft | Machine-readable `io.nexflow.github` profile, repository/revision/review authority boundaries, Software Team binding, and focused checks | Implemented | `extensions/github/`, RFC-0020, `npm run github-extension-smoke` | Policy mapping only; no GitHub client, API binding, token flow, webhook receiver, repository mutation, pull request merge, Actions execution, or live compatibility evidence. |
| MCP extension draft | Machine-readable `io.nexflow.mcp` profile, integration review companion, two declaration fragments, ContextSet boundary, selected Software Team declarations, and 25 focused cases | Implemented | `extensions/mcp/`, RFC-0018, `docs/mcp-integration-profile.md`, `npm run mcp-extension-smoke` | Offline policy mapping and selected structural checks only; no effective-policy evaluator, MCP client, server, transport, discovery, credential, protocol negotiation, or execution support. |
...
| Issue tracker extension draft | Machine-readable `io.nexflow.issue_tracker` profile, closed schema, fictional declaration fragments, and offline boundary checks | Implemented | `extensions/issue-tracker/`, RFC-0021, `npm run issue-tracker-extension-smoke` | Policy assets only; no provider adapter, API binding, live identity resolution, issue mutation, task synchronization, webhook receiver, concurrency enforcement, or runtime conformance. |
...
| Extensions | Core declaration schema, namespace/lifecycle rules, optional registry metadata model, future loading boundary, and maintained experimental GitHub, MCP, A2A, and issue tracker profiles | Partial | `extensions.schema.json`, `docs/extension-registry.md`, `npm run extension-registry-smoke`, `docs/extension-loading-boundary.md`, `extensions/github/`, `extensions/mcp/`, `extensions/a2a/`, `extensions/issue-tracker/`, extension docs, examples | Registry metadata and loading are specified only as bounded draft models; no registry service, published snapshot, ownership verification, loader, live integration, protocol implementation, or plugin execution exists. |
...
github extension profile: extensions/github/profile.yaml
github extension smoke: scripts/github-extension-smoke.mjs
