...
| CLI prototype | Dispatches `discover`, `validate`, `inspect`, `graph`, and `init` through closed immutable effect budgets. | Reads selected manifests; `init` is the only project-file write effect. Every runtime-facing effect is disabled and no command authorizes execution. | [`scripts/lib/cli-boundary.mjs`](../scripts/lib/cli-boundary.mjs), [`scripts/lib/cli-prototype.mjs`](../scripts/lib/cli-prototype.mjs), [`scripts/cli-no-runtime-guardrails-smoke.mjs`](../scripts/cli-no-runtime-guardrails-smoke.mjs) |
...
- Every repository CLI command has a closed effect budget, and focused checks
  verify the reviewed module graph, deny common runtime APIs, preserve read-only
  project snapshots, and constrain initializer writes
  ([CLI Prototype No-Runtime Guardrails](cli-prototype.md#executable-no-runtime-guardrails)).
...
| P2 current | Consumers interpret a structural validation or graph success as proof of semantic correctness, runtime compatibility, or permission to execute. | Output or documentation omits its claim boundary, or downstream automation ignores it. | Unsafe operator decision or misleading conformance claim. | CLI help and every successful output state that no execution is authorized; JSON fixes `executionAuthorized: false`; executable guardrails reject runtime-like commands and run implemented commands with common runtime APIs denied. | Preserve the field, warnings, effect budgets, reviewed module graph, and deny-harness checks as compatibility-sensitive; require conformance claims to name exactly which validation layers ran. | [`scripts/lib/cli-boundary.mjs`](../scripts/lib/cli-boundary.mjs), [`scripts/lib/cli-output.mjs:100-115`](../scripts/lib/cli-output.mjs#L100-L115), [`scripts/cli-no-runtime-guardrails-smoke.mjs`](../scripts/cli-no-runtime-guardrails-smoke.mjs) |
